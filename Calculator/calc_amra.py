...
class Calculator:
    ...
    def login(self):
        if self.name != "":
            return  tkinter.messagebox.showerror("Error", "Already logged in, Logout out to re-login")
        self.name1 = myEntry.get()
        if self.name1 == "":
            return  tkinter.messagebox.showerror("Error", "Enter your name first on the display to login")
        self.profile_login = tkinter.messagebox.askyesnocancel(f"Login in", f"Do you want to login as {self.name1}?")
        if self.profile_login == 1 and self.name == "":
            self.name = self.name1
            menubar.add_cascade(label=f'   Logged in as:  {calcu_obj.name}')
            myEntry.delete(0, "end")
            myEntry.insert(0, "0")
    def logout(self):
        if self.name == "":
            return  tkinter.messagebox.showerror("Error", "You haven't login yet")
        else:
            menubar.delete("end")
            myEntry.delete(0, "end")
            myEntry.insert("end", "0")
            self.name= ""
            self.name1 = ""
            return  tkinter.messagebox.showinfo("Log status", "You have auccessfully logged out")
    ...
    def to_exit(self):
        ...
        self.to_exit = tkinter.messagebox.askyesno("Close AMRA-Calculator?", "Confirm to quit")
        if self.to_exit:
            root.destroy()
    ...
    def help(self):
        #returns true and false
        self.to_exit_help = tkinter.messagebox.showinfo("info/Instruction","1)This is a standard and a scientific calculator \
        \n* Enter your name on the display (i.e AMRA) and then press login on the Profile menubar")
...
login_menu.add_command(label="Login", command=lambda: calcu_obj.login())
login_menu.add_command(label="Logout", command=lambda: calcu_obj.logout())
menubar.add_cascade(label="Log status", menu= login_menu)
